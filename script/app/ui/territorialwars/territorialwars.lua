...
function TerritorialWarUI:createMonster(cellId,info,block)
    ...
    if not bg then
        ...
        local spineAni = ccui.ImageView:create("uires/ui/common/frame_gray.png")
        spineAni:setScale(0.5)
        spineAni:setPosition(cc.p(0, 0))
        spineAni:setAnchorPoint(cc.p(0.5, 0.5))
        bg:addChild(spineAni)

        local imgFrame = ccui.ImageView:create("uires/icon/hero/"..monsterConfig.url.."_icon.png")
        imgFrame:setPosition(cc.p(spineAni:getContentSize().width / 2, spineAni:getContentSize().height / 2))
        imgFrame:setAnchorPoint(cc.p(0.5, 0.5))
        spineAni:addChild(imgFrame)
        ...
        loadingBarBg:setPosition(cc.p(bg:getContentSize().width/2,bg:getContentSize().height+25))
        ...
        loadingBar:setPosition(cc.p(bg:getContentSize().width/2,bg:getContentSize().height+25))
        ...
        lvText:setPosition(cc.p(0,bg:getContentSize().height+30))
        bg:addChild(lvText)
        local zOrder = -info.y
    ...
    MessageMgr:sendPost('gather','territorywar',json.encode({cellId  = cellId}),function (response)
        ...
        if gone == 1 then
            ...
            if self.element[cellId] ~= nil and self.element[cellId]._spine ~= nil and self.element[cellId]._img ~= nil then
                if type(self.element[cellId]._spine.getAnimation) == "function" then
...
function TerritorialWarUI:createMode(gridBlock,v,notOpacity)
    ...
    roleAni:setAnchorPoint(cc.p(0.55, 0))
    roleAni:setPosition(cc.p(obj:getContentSize().width/2, -16))
    obj:addChild(roleAni)
    obj:setAnchorPoint(cc.p(0.5, 0))
    obj:setPosition(cc.p(self.hextab[hexpos.x][hexpos.y]._centerPosX,self.hextab[hexpos.x][hexpos.y]._centerPosY))
