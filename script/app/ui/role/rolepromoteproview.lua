local RolePromotedProviewUI = class("RolePromotedProviewUI", BaseUI)


local herochangeconf = GameData:getConfData('herochange')
local MAXPROTYPE = #herochangeconf

function RolePromotedProviewUI:ctor(obj,protype)
	self.uiIndex = GAME_UI.UI_ROLE_PROMOTED_PROVIEW_PANEL
    self.obj = obj
...
function RolePromotedProviewUI:init()
    ...
    closebtn:addTouchEventListener(function (sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            RoleMgr:hideRolePromotedProviewUI()
        end
    end)
    ...
    titletx:setString(GlobalApi:getLocalStr('ROLE_DESC25'))
...
function RolePromotedProviewUI:addEvent(parent,skillid,pos)
...
function RolePromotedProviewUI:getAtt(protype,lv)
	local objtemp = clone(self.obj)
	local promotedtemp = {}
...
function RolePromotedProviewUI:createAnimation()
...
function RolePromotedProviewUI:initnorpl()
...
function RolePromotedProviewUI:initmaxpl()
    local proattbase = {}
    local att = {}
    local disnumatt = {}
    local basenumatt = {}
