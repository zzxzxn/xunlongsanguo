            ...
            local function sendMsg()
                ...
                MessageMgr:sendPost("swallow", "equip", json.encode(args), function (jsonObj)
                    ...
                    if code == 0 then
                        ...
                        self.swallowEquipData = jsonObj.data
...
function RoleEquipUpgradeStarUI:onMoveOut()
    ...
    if self.swallowEquipData then
        self.equipObj:updateGodAttr(self.swallowEquipData.god, self.swallowEquipData.xp)
        GlobalApi:parseAwardData(self.swallowEquipData.awards)
        if self.swallowEquipData.costs then
            GlobalApi:parseAwardData(self.swallowEquipData.costs)
        end
        local roleobj = RoleData:getRoleByPos(self.equipObj:getPos())
        roleobj:setFightForceDirty(true)
        RoleMgr:updateRoleMainUI()
