...
function ActivityOneYuanBuyUI:init()
	...
	local re1 = xx.RichTextLabel:create("    "..GlobalApi:getLocalStr('ACTIVITY_ONE_YUAN_DES26'), 18, cc.c4b(254,227,134,255))
	re1:setStroke(cc.c4b(140,56,0,255),1)
    re1:setFont('font/gamefont.ttf')

	local re2 = xx.RichTextLabel:create("\n".."    "..GlobalApi:getLocalStr('ACTIVITY_ONE_YUAN_DES27'), 18, COLOR_TYPE.RED)
	--re2:setStroke(cc.c4b(140,56,0,255),1)
    re2:setFont('font/gamefont.ttf')

    local re3 = xx.RichTextLabel:create(GlobalApi:getLocalStr('ACTIVITY_ONE_YUAN_DES28'), 18, cc.c4b(254,227,134,255))
    ...
    buyBtn:addTouchEventListener(function (sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            ...
            if self.data.money_buy.status == 0 then -- 充值
                ...
                RechargeMgr:pay(rechageData[9], function (code)
...
function ActivityOneYuanBuyUI:createItemCell(i,data,symbol)
        local i = tonumber(i)
...
function ActivityOneYuanBuyUI:buyCallback(code)
    -- dump("一元购购买状态为："..code)
