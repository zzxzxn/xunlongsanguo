local HumanWing = class("human_wing")
...
function HumanWing:init(msg)
    self.rootBG = self.root:getChildByName("root")

    self.msg = msg
    UserData:getUserObj().activity.human_wing = self.msg.human_wing
    ...
    self:initLeft()   
    self:updateMark()
    ActivityMgr:showLefHumanWingCue()
end

function HumanWing:updateMark()
    if UserData:getUserObj():getSignByType('human_wing') then
		ActivityMgr:showMark("human_wing", true)
	else
		ActivityMgr:showMark("human_wing", false)
	end
end

function HumanWing:initData()
    self.avData = GameData:getConfData('avhuman_wing')
end

function HumanWing:initTop()
    ActivityMgr:showRightHumanWingRemainTime()
end

function HumanWing:initLeft()
	...
	local re2 = xx.RichTextLabel:create(GlobalApi:getLocalStr('ACTIVITY_HUMAN_WING_DES2'), 26,COLOR_TYPE.GREEN)
...
function HumanWing:updateSV()
    self.verSv:removeAllChildren()
    self.tempData = {}
    local reward = self.msg.human_wing.achieve -- 至少为{}
    ...
    for i = 1,num do
        ...
        getBtn:addTouchEventListener(function (sender, eventType)
            ...
            if eventType == ccui.TouchEventType.ended then
                    MessageMgr:sendPost('get_human_wing_award','activity',json.encode({level = self.tempData[i].level}),
		            function(response)
                        ...
                        self.msg.human_wing.achieve[tostring(self.tempData[i].level)] = 1
                        UserData:getUserObj().activity.human_wing = self.msg.human_wing
        ...
        gotoBtn:addTouchEventListener(function (sender, eventType)
            ...
            if eventType == ccui.TouchEventType.ended then
                local isOpen = GlobalApi:getOpenInfo('wing')
                if not isOpen then
                    promptmgr:showSystenHint(GlobalApi:getLocalStr('ACTIVE_EXCHANGE_POINTS_DES19'), COLOR_TYPE.RED)
                    return
                end
                PeopleKingMgr:showPeopleKingMainUI(2)
                ActivityMgr:hideUI()
            end
        end)
        ...
        local limitLv = self.msg.human_wing.level
        ...
        desc:setString(string.format(GlobalApi:getLocalStr('ACTIVITY_HUMAN_WING_DES4'),self.tempData[i].level))

    end
    innerContainer:setPositionY(size.height - allHeight)
end

function HumanWing:refreshDay(btn)
    local i = btn.i
