...
function PointsExchageAwardUI:ctor(counts,exchangePoints)
    self.uiIndex = GAME_UI.UI_POINTS_EXCHANGE_AWARD
    self.counts = counts
    self.exchangePointsAwardConf = GameData:getConfData('avexchangepointsaward')
...
function PointsExchageAwardUI:init()
    ...
    closeBtn:addTouchEventListener(function (sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            AudioMgr.PlayAudio(11)
			self:hideUI()
            self.exchangePoints:refreshCount(UserData:getUserObj().activity.exchange_points.integral)
        end
    end)
    ...
    for i = 1,num do
        ...
        btn:addTouchEventListener(function (sender, eventType)
            ...
            elseif eventType == ccui.TouchEventType.ended then      
                ...
                MessageMgr:sendPost('get_exchange_points_buy','activity',json.encode({id = confData.id}),
		            function(response)
