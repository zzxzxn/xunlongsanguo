local PageSaleNew = class("PageSaleNew")
local ClassItemCell = require('script/app/global/itemcell')
function PageSaleNew:init(msg)
    ...
    local conf = GameData:getConfData('activities')['value_package_new']
    ...
    ActivityMgr:showRightSaleNewRemainTime()
    ActivityMgr:showLefSDesc2Cue()
    self:registerTouchEvents()

    -- 点击一下这个分页叹号就消失
    UserData:getUserObj().activity.overvalued_gift_new.day = msg.overvalued_gift_new.day

    self:updateMark()

end

function PageSaleNew:updateMark()
    if UserData:getUserObj():getActivitySaleShowStatus() then
        ActivityMgr:showMark("value_package_new", true)
    else
        ActivityMgr:showMark("value_package_new", false)
    end

end

function PageSaleNew:addCells(giftState)
    ...
    local diffTime = time - newOpenServerTime
    local day = math.ceil(diffTime/(24*3600)) - conf.openDay
    print('=========++++++++++++' .. day)

    local saleDatas = GameData:getConfData("avovervaluedgiftnew")
    ...
    for i = 1,num do
        ...
        if(giftState.money >= data.money) then
            if(giftState.overvalued_gift_new[string.format("%d",i)] == nil) then
...
function PageSaleNew:setupCells(giftState)
    local time = Time.getCorrectServerTime()
    local conf = GameData:getConfData('activities')['value_package_new']
...
function PageSaleNew:registerTouchEvents()
...
function PageSaleNew:sendGetGiftMessage(giftID)
    
    MessageMgr:sendPost('get_overvalued_gift_new','activity',json.encode({id = giftID}),function(jsonObj)
