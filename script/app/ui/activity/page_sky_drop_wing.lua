local SkyDropWings = class("SkyDropWings")
local ClassItemCell = require('script/app/global/itemcell')

function SkyDropWings:init(msg)
	...
	titleImg:loadTexture('uires/ui/text/tianjiangshengyi.png')
	self.awardSv = rightBgImg:getChildByName('award_sv')
	self.awardSv:setScrollBarEnabled(false)
	self:createItem()
	self:createRichTextDesc()
	self:updateBtn()
	ActivityMgr:showRightSkyDropWingRemainTime()
end


function SkyDropWings:buy(times)
	local function callback()
		local args = {
			times =  times
		}
		MessageMgr:sendPost("draw_sky_drop_wing",'activity',json.encode(args),function(jsonObj)
	...
	local nums = {
		[1] = tonumber(GlobalApi:getGlobalValue('skyWingCost')),
		[10] = tonumber(GlobalApi:getGlobalValue('skyWingCost'))*10*tonumber(GlobalApi:getGlobalValue('skyWingDiscount'))
	}
...
function SkyDropWings:updateBtn()
	local nums = {
		tonumber(GlobalApi:getGlobalValue('skyWingCost')),
		tonumber(GlobalApi:getGlobalValue('skyWingCost'))*10*tonumber(GlobalApi:getGlobalValue('skyWingDiscount'))
	}
	local rightBgImg = self.rootBG:getChildByName('right_bg_img')
	local times = {1,10}
	for i=1,2 do
...
function SkyDropWings:createRichTextDesc()
	...
	local re2 = xx.RichTextLabel:create(GlobalApi:getLocalStr('SKY_DROP_DESC9'), 26, COLOR_TYPE.ORANGE)
	...
	local re2 = xx.RichTextLabel:create(GlobalApi:getLocalStr('SKY_DROP_DESC10'), 24, COLOR_TYPE.ORANGE)
...
function SkyDropWings:updateMark()

end

function SkyDropWings:onShow()
end

function SkyDropWings:createItem()
	local conf = GameData:getConfData("avskywingheaven")
