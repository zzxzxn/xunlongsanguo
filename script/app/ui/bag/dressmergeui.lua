...
function DressMergeUI:ctor(currId,nextId)
...
function DressMergeUI:OnMerge()
	local currConf = GameData:getConfData("dress")[self.currId]
    local costNum = self.times*currConf.mergeGold
	UserData:getUserObj():cost('gold',costNum,function()
	 	local args = {
	  		did = self.obj:getId(),
	  		num = self.times
	  	}
	  	MessageMgr:sendPost('merge_dress','equip',json.encode(args),function (response)
...
function DressMergeUI:update()
    ...
    self.costNumTx:setString(self.times*currConf.mergeGold)
end
...
function DressMergeUI:init()
	...
	titleTx:setString(GlobalApi:getLocalStr('DRESS_MERGE'))
	...
	self.obj = BagData:getDressByIdForShop(self.currId)
	self.nextObj = BagData:getDressByIdForShop(self.nextId)
	...
	local currConf = GameData:getConfData("dress")[self.currId]
	local nextConf = GameData:getConfData("dress")[self.nextId]
